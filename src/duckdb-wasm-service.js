...
function getQueryParams() {
  ...
  return {
    ...
    collection: params.get('collection') || null, // optional collection id/alias to list
...
async function getArtifactManager() {
  ...
  artifactManager = await server.getService('public/artifact-manager', { case_conversion: 'camel' });
...
async function listArtifacts() {
  ...
  let collId = collection;
  if (!collId) {
    // Default to user's workspace datasets collection if present; fallback to workspace root
    collId = "biomni-dataset-collection";
  }
  try {
    const artifacts = await am.list({ parent_id: collId || null, _rkwargs: true });
...
function renderArtifacts(items) {
  ...
  container.innerHTML = '';
  if (!items.length) {
    container.innerHTML = '<div style="color:#8b8b8b">No artifacts found.</div>';
    return;
  }
  items.forEach((it, idx) => {
    const div = document.createElement('div');
    div.className = 'artifact-item' + (currentArtifact && currentArtifact.id === it.id ? ' active' : '');
...
async function selectArtifact(artifact) {
  ...
  document.getElementById('tableHint').textContent = `Loaded table name: ${tableName} from ${artifact.manifest?.name || artifact.id}`;
  try {
    await loadArtifactIntoDuckDB(artifact);
  } catch (e) {
    document.getElementById('resultsContainer').innerHTML = `<div style="color:#f48771">${e.message}</div>`;
  }
}
...
async function loadDuckDB() {
  ...
  const mod = await import('https://cdn.jsdelivr.net/npm/@duckdb/duckdb-wasm@1.29.0/dist/duckdb-browser.mjs');
  ...
  try {
    ...
    const usingLocalBundle = bundle.mainWorker && bundle.mainWorker.startsWith(location.origin);
    if (isLocal && !usingLocalBundle) throw new Error('Force main-thread on localhost without local bundle');
...
async function getArtifactFile(artifactId, path) {
  ...
  const url = await am.getFile({ artifact_id: artifactId, file_path: path, _rkwargs: true });
...
async function previewTable(limit = 50) {
  ...
  const columns = res.getColumnNames ? res.getColumnNames() : (rows[0] ? Object.keys(rows[0]) : []);
...
async function runSql(sql) {
  ...
  if (res.numRows === 0) {
    document.getElementById('resultsContainer').innerHTML = '<div style="color:#8b8b8b">OK</div>';
    return;
  }
  const rows = res.toArray();
  const columns = res.getColumnNames ? res.getColumnNames() : (rows[0] ? Object.keys(rows[0]) : []);
...
async function resolveArtifact(artifactIdOrName) {
  ...
  if (!hit) hit = items.find(x => (x.manifest?.name || '').toLowerCase() === String(artifactIdOrName).toLowerCase());
  ...
  const svc = await server.registerService({
    ...
    register_functions: {
      ...
      query: {
        ...
        params: [{ name: 'artifact', type: 'string' }, { name: 'sql', type: 'string' }],
        returns: { type: 'object' },
      },
    },
  });
...
function wireUi() {
  document.getElementById('connectBtn').addEventListener('click', async () => {
    ...
    try { await registerService(); } catch (e) { console.warn('Service registration failed:', e); }
    document.getElementById('refreshArtifactsBtn').disabled = false;
    const items = await listArtifacts();
    renderArtifacts(items);
  });
  document.getElementById('refreshArtifactsBtn').addEventListener('click', async () => {
    const items = await listArtifacts();
    renderArtifacts(items);
  });
  ...
  document.getElementById('showSchemaBtn').addEventListener('click', async () => {
    try {
      const res = await conn.query(`PRAGMA table_info(${tableName});`);
      const rows = res.toArray();
      const columns = res.getColumnNames ? res.getColumnNames() : (rows[0] ? Object.keys(rows[0]) : []);
